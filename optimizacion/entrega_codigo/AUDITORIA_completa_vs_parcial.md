...
`fitness_function` de ambos modelos devuelve un float finito sobre los
parametros de referencia (completo = 0.036594, simplificado = 0.043637).
...
- Pesos: `W_IFP = 0.40`, `W_IFD = 0.60`, `W_MONO = 5.0` (lineas 78-80).
- Regularizacion suave: `W_REG_DIM = 5e-4`, `W_REG_AUX = 5e-4` (lineas 83-84).
- `fitness_function` (lineas 232-254): alinea rigidamente mocap contra sim
...
- Pesos: `W_IFP = 0.25`, `W_IFD = 0.375`, `W_TIP = 0.375`, `W_MONO = 5.0`,
  `W_DIP = 0.3` (lineas 96-100).
- Misma regularizacion suave: `W_REG_DIM = 5e-4`, `W_REG_AUX = 5e-4`
  (lineas 102-103).
- `fitness_function` (lineas 256-291): misma alineacion rigida; calcula
  ...
  (lineas 275-277) escalado por `W_DIP`; suma
...
(lineas 133-143) y la usan ambos modelos con el mismo peso `W_MONO = 5.0`:

- Parcial: `mono_ifd = monotonicity_penalty(aligned['ifd'])` y
  `mono_error = W_MONO * mono_ifd` (`modelo_simplificado.py`, lineas 244 y 247).
...
`modelo_simplificado.py` linea 205 (dentro de `run_kinematics`) y
`modelo_completo.py` lineas 190-193.
...
- Parcial (`optimizar_simplificado_ED.py`, lineas 47-51):
  `N_TRIALS_OPTUNA=25`, `DE_MAXITER_OPTUNA=40`, `OPT_MAXITER=300`, `OPT_SEED=42`,
  `OPTUNA_SEED=42`.
- Completa (`optimizar_completo_ED.py`, lineas 51-56): `N_TRIALS_OPTUNA=25`,
...
(`optimizar_simplificado_ED.py` lineas 74-82; `optimizar_completo_ED.py`
lineas 87-95).
...
(`optimizar_simplificado_ED.py` lineas 96-97; `optimizar_completo_ED.py`
lineas 109-110).
...
lineas 60-77 y 96-98). El docstring justifica que el pulido local L-BFGS es
...
sin el argumento, `optimizar_completo_ED.py` lineas 143-145). En la parcial, la
ED usa `polish=True` siempre (`optimizar_simplificado_ED.py` lineas 55-66). Esta
...
(`optimizar_tercer_mecanismo.py` lineas 48-50 para los defaults por entorno y
lineas 74-80 para la llamada a `differential_evolution`). Importa el modelo
...
- `gear_ratio <= 0`: `modelo_simplificado.py` linea 118; `modelo_completo.py`
  linea 121.
- longitudes minimas `min(p[:11]) <= 0.005`: `modelo_simplificado.py` linea 120;
  `modelo_completo.py` linea 123.
- `hsp <= 0 or dsp <= 0`: `modelo_simplificado.py` linea 122;
  `modelo_completo.py` linea 128.
- `FP_REAL - 2.0*dsp <= 0.001`: `modelo_simplificado.py` linea 124;
  `modelo_completo.py` linea 130.

Restriccion anti-gancho de la medial (`delta_fm >= -0.5 deg`):
`modelo_simplificado.py` lineas 202-206; `modelo_completo.py` lineas 190-193.

Chequeos de finitud/rango de la trayectoria de salida (`abs > 0.3` o no finito
-> `None`): `modelo_simplificado.py` lineas 210-212 (IFD);
`modelo_completo.py` lineas 225-227 (punta).
...
(`modelo_simplificado.py` lineas 288-331; `modelo_completo.py` lineas 320-362).

Penalizacion fija de 1000: `return 1000.0` cuando `run_kinematics` regresa
`None` en `fitness_function` de ambos (`modelo_simplificado.py` linea 235;
`modelo_completo.py` linea 259).

Extensiones de la completa (falange distal):

- Validaciones extra del tercer mecanismo: `Link9_3 <= 0.005 or Link10_3 <= 0.005`
  y `up3_3 <= 0` (`modelo_completo.py` lineas 125-127).
- Tercer mecanismo de cuatro barras: calcula el punto de anclaje `Pa`, resuelve
  ...
  lineas 195-217). Si los circulos no intersectan, regresa `None`
  (linea 212-213), lo que dispara la penalizacion 1000.
- Tope fisiologico DIP: `DIP_MAX_DEG = 35.0` (linea 32) y la restriccion
  `if np.ptp(dip_rel) > np.deg2rad(DIP_MAX_DEG): return None`
  (lineas 231-233), que tambien deriva en penalizacion 1000.
...
- La cinematica del tercer mecanismo usa cuatro parametros dimensionales
  ...
  standoff dorsal; ver bounds en `modelo_completo.py` lineas 88-91 y NOMBRES en
  lineas 76-79) y resuelve el lazo con `circle_intersections`
  (`modelo_completo.py` lineas 210-217).
...
- `exo_18_pinza_fina.py` (modelo legacy) lo describe igual: lineas 195, 221,
  ...
  `_circle_intersections` (linea 372) y la calibracion `gamma_bracket3`
  (lineas 380-383).
...
- (a) Funcion objetivo: los pesos y la estructura son consistentes
  (`modelo_simplificado.py` lineas 78-84 y 232-254; `modelo_completo.py`
  lineas 96-103 y 256-291). La completa extiende, no contradice.
- (b) Monotonicidad: misma funcion y peso `W_MONO=5.0`
  (`comun.py` lineas 133-143; `modelo_simplificado.py` lineas 244-247;
  `modelo_completo.py` lineas 272-280).
- (c) Optuna: mismos trials (25), mismas semillas (42, 42) y mismos rangos de
  sugerencia (`optimizar_simplificado_ED.py` lineas 47-51 y 74-97;
  `optimizar_completo_ED.py` lineas 51-56 y 87-110); la unica diferencia
...
- `modelo_completo.fitness_function(p_ref)` = 0.036594 (finito).
- `modelo_simplificado.fitness_function(p_ref)` = 0.043637 (finito).
